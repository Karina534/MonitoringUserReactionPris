...
### Разделение на Бейзлайн (Baseline) и Основной MVP

Для валидации эффективности внедрения методов машинного обучения в системе жестко разделяются две архитектуры: бейзлайн и целевой MVP.

* **Бейзлайн (Baseline):** Детерминированный алгоритм на основе регулярных выражений (RegEx) и статического словаря запрещенных стоп-слов/матерных корней (библиотека `pymorphy2` + алгоритм Ахо-Корасик для быстрого поиска подстрок).
   * *Плюсы:* Нулевая стоимость вычислений, инференс < 5 мс, 100% интерпретируемость.
   * *Минусы:* Полное отсутствие контекстуального понимания. Легко обходится умышленными опечатками (обфускацией), не определяет скрытый негатив, сарказм или токсичность. Дает критически низкий Recall (~45%).
* **Основной MVP:** Контекстная языковая модель **DistilRuBERT**, предобученная на русскоязычном корпусе текстов, квантованная до формата INT8 и запущенная на движке ONNX Runtime.
   * *Плюсы:* Высокий Recall ($\ge 96\%$), понимание семантики предложения, устойчивость к синонимам и неявной агрессии.
   * *Минусы:* Требует GPU-инфраструктуры для обучения и оптимизированного инференса, имеет вероятностную природу.

**Почему MVP станет лучше, чем Бейзлайн:**
Бейзлайн за счет пропусков (FN) не способен защитить компанию от регуляторных штрафов РКН при усложнении лексики пользователей. MVP за счет механизма Self-Attention в трансформерах улавливает структуру связей между словами, обеспечивая прирост Recall более чем на 50% по сравнению со словарным подходом.
...
### Блок-схема логики работы: Бейзлайн vs MVP

![img.png](../assets/baseline.png)
...
## Спецификация входных данных (Подготовка данных)

### Источники данных для обучения модели

| Название таблицы / сущности | Описание и фичи для обучения | Наличие | Источник / Витрина | Требуемый ресурс | QA |
|:---------------------------|:----------------------------|:-------:|:------------------|:----------------|:---|
| **train_comments_gold** | Исторические тексты комментов с меткой разметки (токсичный/нет). Целевая переменная. | Да | DATAMARTS.moderation_history_v1 | Дата-аналитик, Дата-инженер | Проверено. Исключены дубликаты. Дисбаланс: 7% токсичных |
| **user_meta_features** | Метаданные автора (возраст аккаунта, история банов). Контекст. | Да | PRODUCTION.users_profile | Дата-инженер, Администратор БД | Пропуски в геопозиции. Для MVP: только is_previously_banned |
| **rkn_stop_words** | Официальный список запрещенных слов и паттернов регулятора | Да | LEGAL.rkn_blacklist_v2 | Юридический отдел, Владелец продукта | Проверено юристами. Обновляется раз в квартал |
...
### Оценка вычислительной сложности (Computational Complexity)

Исходя из бизнес-требования обрабатывать 500 000 комментариев в день при пиковых нагрузках до 100 RPS, произведен расчет необходимых вычислительных мощностей для инференса MVP (DistilRuBERT, ~65 млн параметров):

1.  **Размер батча (Batch Size):** В стриминг-режиме обработка идет поштучно (`batch_size = 1`).
2.  **Затраты на один инференс (FP16):** Около 0.8 GFLOPs на одно сообщение.
3.  **Требования к железу при пике 100 RPS:**
   * При инференсе на **CPU (Intel Xeon Gold 6230)** средняя задержка составляет ~120 мс. Один поток процессора может обработать ~8 RPS. Для 100 RPS потребуется кластер из 12-14 ядер CPU.
   * При инференсе на **GPU (NVIDIA T4)** с использованием тензорных ядер (TensorRT FP16) задержка падает до **8-12 мс**. Одна карта NVIDIA T4 способна выдавать до 800 RPS для данной модели.
4.  **Решение для MVP:** Использование **1x NVIDIA T4** (или аналога в облаке) с запасом по пропускной способности в 8 раз, что полностью нивелирует риск задержек при резких всплесках активности пользователей.
...
## Безопасность и издержки (Production стандарты)

### Безопасность системы (System Security & Vulnerabilities)
ML-инференс сервис подвержен специфичным ИБ-уязвимостям, для защиты от которых внедрены следующие механизмы:
* **Защита от DOS-атак на инференс (Slowing Down Attacks):** Злоумышленники могут отправлять сверхдлинные текстовые сообщения (например, книги в одном комментарии), чтобы перегрузить память GPU. *Решение:* На уровне Preprocessing Service установлено жесткое ограничение сверху — тексты длиннее 1000 символов автоматически обрезаются или отклоняются до подачи в нейросеть.
* **Data Poisoning (Отравление данных):** Риск умышленной порчи обучающей выборки через отправку скоординированных комментариев. *Решение:* Логи разметки модераторов (`labeling_logs`), идущие на дообучение, проходят обязательную кросс-валидацию (один и тот же текст должны независимо разметить минимум 2 старших модератора).

### Безопасность данных и комплаенс (Data Privacy, GDPR, 152-ФЗ)
* **Деперсонализация контента:** Пайплайн обработки данных внутри подсистемы анализа комментариев не оперирует персональными данными (ПДн). Сервис предобработки на этапе Kafka-consumer'а удаляет из текста номера телефонов, email-адреса и номера банковских карт с помощью регулярных выражений перед отправкой текста в модель.
* **Логирование:** В базы данных `ml_verdicts` и `prediction_logs` записываются только `comment_id`, маскированный текст и скоры модели. Связка с реальным ФИО пользователя остается внутри изолированного контура PostgreSQL основного бэкенда соцсети, что полностью удовлетворяет требованиям **ФЗ-152 «О персональных данных»**.

### Расчет финансовых издержек на работу системы (Cost Estimation)
Примерная оценка операционных затрат (OpEx) на содержание инфраструктуры в месяц (на базе тарифов российских облачных провайдеров):

1.  **Вычислительный контур ML (Inference):**
   * 1x Нода с GPU NVIDIA T4 (vCPU: 8, RAM: 32 GB) = **~15 000 руб./мес.**
2.  **Контур баз данных и очередей:**
   * Managed Kafka (3 ноды для отказоустойчивости) = **~12 000 руб./мес.**
   * Managed PostgreSQL (Основная база, реплика + бэкапы) = **~8 000 руб./мес.**
   * ClickHouse (Аналитическое хранилище логов, 1 нода, 1 ТБ диск) = **~7 000 руб./мес.**
3.  **Облачное хранилище S3 (Хранение моделей и датасетов, ~500 ГБ):** = **~1 500 руб./мес.**

**Итоговые ориентировочные затраты на инфраструктуру MVP:** **~43 500 рублей в месяц.** Данная сумма является экономически оправданной, так как один потенциальный штраф РКН за несвоевременное удаление противоправного контента может составлять от 800 000 до 4 000 000 рублей.
...
```text
@startuml
title Блок-схема маршрутизации: Сравнение Baseline и MVP

start
:Входящий комментарий;
:Препроцессинг и очистка текста;

note left
  Шаг 1: Проверка через
  Бейзлайн (Словари и RegEx)
end note
if (Найдено прямое совпадение со стоп-словом?) then (Да (Мат/Спам))
    :Автоматический бан (Вердикт Baseline);
    :Статус = hidden / deleted;
    stop
else (Нет прямых совпадений)
    note right
      Шаг 2: Глубокий анализ
      через MVP (DistilRuBERT)
    end note
    :Токенизация и инференс модели;
    :Расчет вероятности Toxicity Score;
    
    if (Toxicity Score > 0.85) then (Критический риск)
        :Автоматическая блокировка;
    else if (Toxicity Score между 0.50 и 0.85) then (Серая зона)
        :Создание кейса в moderation_cases;
        :Передача модератору (до 5 сек);
    else (Безопасно)
        :Статус = approved;
    endif
endif
